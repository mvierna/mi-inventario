...
def quitar_cero_inicial(codigo):
    """
    Función de apoyo: Si un código EAN tiene 13 dígitos y empieza por '0',
...
def procesar_inventario():
    ...
    # Leemos las 4 columnas de TIENDA.csv como texto estricto
    ...
    # Limpiamos nulos y posibles decimales sobrantes
    df['EAN'] = df['EAN'].fillna('')
    df['EAN'] = df['EAN'].astype(str).str.replace('.0', '', regex=False).str.strip()
    
    # Eliminamos cualquier cabecera previa o fila vacía
    df = df[~df['EAN'].str.lower().isin(['ean', 'code', 'col_a', ''])]

    # 3. Creación del conjunto de 13 dígitos (con cero a la izquierda)
    df_13 = df.copy()
    df_13['EAN'] = df_13['EAN'].str.zfill(13)

    # 4. Creación del conjunto de 12 dígitos (para el escáner)
    df_12 = df_13.copy()
    df_12['EAN'] = df_12['EAN'].apply(quitar_cero_inicial)

    # Combinamos ambas listas preservando la columna A original
    ...
    print(f"3. Sobrescribiendo '{nombre_archivo}' con formato compatible para la app...")
    # Guardamos en TIENDA.csv asegurando delimitador ';' y cabecera explícita
    df_final.to_csv(nombre_archivo, index=False, header=True, sep=';', encoding='utf-8')

    print("4. Sincronizando datos con la nube mediante Git...")
    try:
        subprocess.run(['git', 'add', '.'], check=True)
        subprocess.run(['git', 'commit', '-m', 'Sincronizacion completa de inventario con cabeceras'], check=True)
        subprocess.run(['git', 'push'], check=True)
        print("\n¡[ÉXITO] TIENDA.csv actualizado correctamente en la nube!")
